...
if exist "%FROMBASEDIR%" goto if1
  echo ***Error: from base directory %FROMBASEDIR% does not exist
  set error=1
:if1
if exist "%TOBASEDIR%" goto if2
  echo ***Error: from base directory %TOBASEDIR% does not exist
  set error=1
:if2
...
copy %FROMBASEDIR%\%FROMDIR%\%FROMFILE% %TOBASEDIR%\%TODIR%\%FROMFILE%
exit /b 1
