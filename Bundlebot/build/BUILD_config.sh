#!/bin/bash
# This scripts defines revisions and tags for a bundle.
# It is run by the other BUILD scripts. 
# You do not need to run it.

export BUNDLE_FDS_REVISION=a33726c82
export BUNDLE_FDS_TAG=FDS-6.9.0

export BUNDLE_SMV_REVISION=d5e9e21f2
export BUNDLE_SMV_TAG=SMV-6.9.0

export BUNDLE_CAD_REVISION=c5fd24b
export BUNDLE_CAD_TAG=CAD-6.9.0

export BUNDLE_EXP_REVISION=8e390fd56
export BUNDLE_EXP_TAG=EXP-6.9.0

export BUNDLE_FIG_REVISION=933b82a
export BUNDLE_FIG_TAG=FIG-6.9.0

export BUNDLE_OUT_REVISION=6b7ac2e39
export BUNDLE_OUT_TAG=OUT-6.9.0

# lines below should not need to be changed

export GH_REPO=test_bundles
export GH_FDS_TAG=BUNDLE_TEST
export GH_SMOKEVIEW_TAG=BUNDLE_TEST
export BUNDLE_FDS_REVISION=a33726c82
export BUNDLE_FDS_TAG=FDS-6.9.0

export BUNDLE_SMV_REVISION=d5e9e21f2
export BUNDLE_SMV_TAG=SMV-6.9.0

export BUNDLE_CAD_REVISION=c5fd24b
export BUNDLE_CAD_TAG=CAD-6.9.0

export BUNDLE_EXP_REVISION=8e390fd56
export BUNDLE_EXP_TAG=EXP-6.9.0

export BUNDLE_FIG_REVISION=22e912e
export BUNDLE_FIG_TAG=FIG-6.9.0

export BUNDLE_OUT_REVISION=6b7ac2e39
export BUNDLE_OUT_TAG=OUT-6.9.0

