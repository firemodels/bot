...
function usage {
echo "This script outputs an fds or smv repo revision"
echo ""
echo "Options:"
...
error_log=/tmp/get_rev_error_$USER.log
to_dir=/tmp
SHOWPARMS=
...
if [ "$repo_type" == "fds" ]; then
  CP FDS_REVISION $to_dir/FDS_REVISION.$$ || exit 1
  cat $to_dir/FDS_REVISION.$$
  rm -f $to_dir/FDS_REVISION.$$
fi

if [ "$repo_type" == "smv" ]; then
  CP SMV_REVISION $to_dir/SMV_REVISION.$$ || exit 1
  cat $to_dir/SMV_REVISION.$$
  rm -f $to_dir/SMV_REVISION.$$
fi
