...
This document describes the processes used during the development and deployment of the Fire Dynamics Simulator (FDS), the Consolidated Fire and Smoke Transport Model (CFAST), and the companion program Smokeview.  This software quality assurance (SQA) plan is intended to guide the planning for modifications to the models, provide required reviews for both software and associated documentation of the model, define testing to be conducted prior to the release of an updated model, describe problem reporting and resolution procedures, and ensure all records, source code, and released software is kept available for the life of the code.  While this report and many of our practices follow the ASTM International Standard 1355-05, ``Standard Guide for Evaluating the Predictive Capability of Deterministic Fire Models'' \cite{ASTM:E1355} which has been used extensively to guide the documentation, verification, and validation of the model, other standards have been followed as well.  Most notably, Institute of Electrical and Electronics Engineers (IEEE) standards IEEE 730-2002 \cite{IEEE:730} and 828~\cite{IEEE-828} define best practices in important areas of software quality assurance and guides the structure and content of this plan. Software development and testing closely align with best practices for scientific computing described in reference \cite{Best_Practices}.

The volumes that make up the FDS and CFAST documentation are based in part on ASTM E 1355, {\em Standard Guide for Evaluating the Predictive Capability of Deterministic Fire Models}~\cite{ASTM:E1355}. ASTM~E~1355 outlines the process of assessing the accuracy of a fire model. Volumes~1 through 3 are the result of this evaluation process. The main purpose of the present volume is to describe the process by which the model software is developed and maintained. These models do not remain static. As progress is made in fire science, the models may be updated and improved, but still need to be shown to reliably predict the kinds of phenomena for which it was originally designed.  Thus, the NIST models are purposely viewed as both production codes (i.e., ones that have well-documented and tested uses) and as research codes (i.e., ones where features are included that have not yet been fully documented and tested). It is the user's responsibility to ensure that the application of the models to a specific scenario is appropriate and within the stated range of applicability of the model. This Guide defines the process by which NIST develops and tests the models during the development process and prior to release of a specific production version of the code. In part, this testing process provides quantitative data that provides guidance to assess the applicability of the models to a range of scenarios.
...
\item{Suppression} Suppression of fires in CFAST is based on a simple empirical correlation for sprinkler suppression of fires in open spaces.  The effects of multiple sprinklers or where the fire may be shielded from the direct sprinkler spray are not modeled.
...
\bibliography{../Bibliography/SQA_refs}
