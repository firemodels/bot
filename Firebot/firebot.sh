...
check_compile_fds_mpi_db()
{
  ...
  if [[ `grep -E -i 'warning|remark' $OUTPUT_DIR/stage2b${MPTYPE} | grep -v mpiifort | grep -v mpiifx | grep -v 'pointer not aligned at address' | grep -v ipo | grep -v Referenced | grep -v atom | grep -v 'is now deprecated'| grep -v 'feupdateenv is not implemented'` == "" ]]
  then
      # Continue along
      :
  else
     echo "Warnings from Stage 2b - Compile FDS MPI debug:" >> $WARNING_LOG
     grep -A 5 -E -i 'warning|remark' $OUTPUT_DIR/stage2b${MPTYPE} | grep -v mpiifort | grep -v mpiifx | grep -v 'pointer not aligned at address' | grep -v ipo | grep -v Referenced | grep -v atom | grep -v 'feupdateenv is not implemented' >> $WARNING_LOG
     echo "" >> $WARNING_LOG
  fi
}
