...
# Check for errors in SMV debug compilation
 ...
 # Check for compiler warnings/remarks
 # grep -v 'feupdateenv ...' ignores a known FDS MPI compiler warning (http://software.intel.com/en-us/forums/showthread.php?t=62806)
 if [[ `grep -E -i 'warning' $OUTPUT_DIR/stage3b | grep -v 'feupdateenv is not implemented' | grep -v 18020 | grep 'was built' | grep -v 'lcilkrts linked'` == "" ]]; then
   # Continue along
   :
 else
   echo "Warnings from Stage 3b - Compile SMV debug:" >> $WARNING_LOG
   grep -A 5 -E -i 'warning' $OUTPUT_DIR/stage3b | grep -v 'feupdateenv is not implemented' | grep -v 18020 | grep 'was built' | grep -v 'lcilkrts linked' >> $WARNING_LOG
   echo "" >> $WARNING_LOG
 fi
}
...
check_compile_smv()
{
  ...
  # Check for compiler warnings/remarks
  # grep -v 'feupdateenv ...' ignores a known FDS MPI compiler warning (http://software.intel.com/en-us/forums/showthread.php?t=62806)
  if [[ `grep -E -i 'warning' $OUTPUT_DIR/stage3c | grep -v 'was built for newer' | grep -v 18020 | grep -v 'was built' | grep -v 'feupdateenv is not implemented' | grep -v 'lcilkrts linked'` == "" ]]; then
    # Continue along
    :
  else
    echo "Warnings from Stage 3c - Compile SMV release:" >> $WARNING_LOG
    grep -A 5 -E -i 'warning' $OUTPUT_DIR/stage3c | grep -v 'was built for new' | grep -v 18020 | grep -v 'was built' | grep -v 'feupdateenv is not implemented' | grep -v 'lcilkrts linked' >> $WARNING_LOG
    echo "" >> $WARNING_LOG
  fi
  smv_release_success=true
}
...
# compare images
    ...
    CURDIR=`pwd`
    cd $botrepo/Firebot
    ./compare_images.sh $figrepo/fds/Reference_Figures $FDS_SUMMARY_DIR/images $FDS_SUMMARY_DIR/diffs/images $OUTPUT_DIR/error_images >& $OUTPUT_DIR/stage8_image_compare
